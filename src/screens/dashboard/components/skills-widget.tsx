...
export function SkillsWidget({
  ...
  return (
    <DashboardGlassCard
      ...
      draggable={draggable}
      onRemove={onRemove}
      className="h-full rounded-xl border-neutral-800 bg-neutral-900 p-4 sm:p-5 shadow-[0_6px_20px_rgba(0,0,0,0.25)] [&_h2]:text-[11px] [&_h2]:font-medium [&_h2]:uppercase [&_h2]:tracking-wider [&_h2]:text-neutral-500 [&_h2]:text-balance [&_svg]:text-neutral-500"
    >
      {skillsQuery.isLoading && skills.length === 0 ? (
        <div className="flex h-28 items-center justify-center gap-3 rounded-lg border border-neutral-800 bg-neutral-950">
          <span
            className="size-4 animate-spin rounded-full border-2 border-neutral-700 border-t-neutral-300"
            role="status"
            aria-label="Loading"
          />
          <span className="text-sm text-neutral-400">Loading skills…</span>
        </div>
      ) : skills.length === 0 ? (
        <div className="flex h-28 flex-col items-center justify-center gap-1 rounded-lg border border-neutral-800 bg-neutral-950">
          <p className="text-sm font-semibold text-neutral-100">No skills installed</p>
            ...
            return (
              <article
                ...
                className={cn(
                  ...
                  'rounded-lg border border-neutral-800 px-2.5 py-2 md:px-3 md:py-2.5',
                  index % 2 === 0 ? 'bg-neutral-950' : 'bg-neutral-950/80',
                )}
              >
                <div className="flex items-start justify-between gap-3">
                  <p className="line-clamp-1 text-xs md:text-sm font-medium text-neutral-100 text-balance">
                    {skill.name}
                  </p>
                  <span
                    className={cn(
                ...
                <p className="mt-1 line-clamp-1 md:line-clamp-2 text-xs md:text-sm text-neutral-400 text-pretty">
                  {skill.description}
                </p>
              </article>
            )
      ...
      <Button
        variant="outline"
        className="mt-3 w-full justify-between rounded-lg border-neutral-800 bg-neutral-950 text-neutral-200 hover:bg-neutral-900"

...
type UsageData = {
  ...
  usage?: { totals?: Totals; startDate?: string; endDate?: string; updatedAt?: number }
}
...
function StatCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-lg border border-primary-200 p-4">
      <div className="text-[11px] font-medium text-primary-500 uppercase tracking-wider">{label}</div>
      <div className="text-xl font-semibold text-ink mt-1">{value}</div>
      {sub ? <div className="text-[11px] text-primary-500 mt-0.5">{sub}</div> : null}
    </div>
  )
}
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto px-6 py-4">
        {query.isLoading ? (
          ...
          <>
            {period ? <p className="text-[11px] text-primary-500 mb-4">{period}</p> : null}

            {/* Summary cards */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
              <StatCard label="Total Cost" value={formatCost(usage?.totalCost)} sub={cost ? `Session: ${formatCost(cost.totalCost)}` : undefined} />
              <StatCard label="Total Tokens" value={formatTokens(usage?.totalTokens)} />
              <StatCard label="Input Cost" value={formatCost(usage?.inputCost)} />
              <StatCard label="Output Cost" value={formatCost(usage?.outputCost)} />
            </div>

            {/* Breakdown table */}
            <h2 className="text-[13px] font-semibold text-ink mb-3">Cost Breakdown</h2>
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-primary-200 text-left">
                  <th className="pb-2 text-[11px] font-medium text-primary-500 uppercase tracking-wider">Category</th>
                  <th className="pb-2 text-[11px] font-medium text-primary-500 uppercase tracking-wider text-right">Tokens</th>
                  <th className="pb-2 text-[11px] font-medium text-primary-500 uppercase tracking-wider text-right">Cost</th>
                </tr>
              </thead>
              <tbody>
                {[
                  { label: 'Input', tokens: usage?.input, cost: usage?.inputCost },
                  { label: 'Output', tokens: usage?.output, cost: usage?.outputCost },
                  { label: 'Cache Read', tokens: usage?.cacheRead, cost: usage?.cacheReadCost },
                  { label: 'Cache Write', tokens: usage?.cacheWrite, cost: usage?.cacheWriteCost },
                ].map((row) => (
                  <tr key={row.label} className="border-b border-primary-100 hover:bg-primary-50">
                    <td className="py-2.5 text-ink">{row.label}</td>
                    <td className="py-2.5 text-primary-600 text-right tabular-nums">{formatTokens(row.tokens)}</td>
                    <td className="py-2.5 text-primary-600 text-right tabular-nums">{formatCost(row.cost)}</td>
                  </tr>
                ))}
                <tr className="font-medium">
                  <td className="py-2.5 text-ink">Total</td>
                  <td className="py-2.5 text-ink text-right tabular-nums">{formatTokens(usage?.totalTokens)}</td>
                  <td className="py-2.5 text-ink text-right tabular-nums">{formatCost(usage?.totalCost)}</td>
                </tr>
              </tbody>
            </table>
          </>

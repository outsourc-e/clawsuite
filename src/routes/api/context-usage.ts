      ...
      GET: async () => {
        try {
          const [sessionsResult, usageResult] = await Promise.allSettled([
            ...
            gatewayRpc<any>('sessions.usage', { limit: 10, includeContextWeight: true }),
          ])

          const sessions = sessionsResult.status === 'fulfilled'
            ? (sessionsResult.value?.sessions ?? [])
            : []
          const usageSessions = usageResult.status === 'fulfilled'
            ? (usageResult.value?.sessions ?? [])
            : []

          const mainSession = sessions.find((s: any) =>
            s.kind === 'main' || (s.key && s.key.includes(':main')),
          )

          if (!mainSession) {
            return json({ ok: true, contextPercent: 0, model: '', maxTokens: 0, usedTokens: 0 })
          }

          const model = mainSession.model ?? ''
          const maxTokens = getContextWindow(model)
          const mainUsage = usageSessions.find((s: any) => s.key === mainSession.key)
          ...
          if (cw) {
            ...
            const toolsChars = (cw.tools?.listChars ?? 0) + (cw.tools?.schemaChars ?? 0)
            staticTokens = Math.ceil((systemChars + skillsChars + toolsChars) / CHARS_PER_TOKEN)
          }
          ...
          const usedTokens = Math.min(staticTokens + conversationTokens, maxTokens)
          const contextPercent = maxTokens > 0 ? (usedTokens / maxTokens) * 100 : 0
          ...
          return json(
            { ok: false, error: err instanceof Error ? err.message : String(err), contextPercent: 0 },
            { status: 500 },
          )
        }
      },
